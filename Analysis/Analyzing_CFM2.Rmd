...
CFM2_4_vars <- filter(CFM2_4_vars, !Child_ID %in% incorrect_screeners$Child_ID)
CFM5_17_vars <- filter(CFM5_17_vars, !Child_ID %in% incorrect_screeners$Child_ID)
...
sum(is.na(CFM2_4_vars)) #0
sum(is.na(CFM5_17_vars)) #2- will keep since it is just for glasses/hearing aid questions
...
# Obtain their means 
CFM2_4_prop_se <- CFM2_4_table1_long %>%
  group_by(Severity) %>%
  summarise(Proportion = mean(Difficulty),
            n = n(),
            se = sqrt(Proportion * (1 - Proportion)/n))

CFM5_17_prop_se <- CFM5_17_table1_long %>%
  group_by(Severity) %>%
  summarise(Proportion = mean(Difficulty),
            n = n(),
            se = sqrt(Proportion * (1 - Proportion)/n))

# Cleaning the tables by turning values into percentages and rounding and creating 95% CI bounds
CFM2_4_CI <- CFM2_4_prop_se %>%
  mutate(Percentage = round(Proportion * 100,2),
         SE = round(se * 100,2),
         Lower = round(Percentage - 1.96 * SE,2),
         Upper = round(Percentage + 1.96 * SE,2))

CFM5_17_CI <- CFM5_17_prop_se %>%
  mutate(Percentage = round(Proportion * 100,2),
         SE = round(se * 100,2),
         Lower = round(Percentage - 1.96 * SE,2),
         Upper = round(Percentage + 1.96 * SE,2))
...
CFM2_4_CI_ordered <- CFM2_4_CI[match(CFM2_4_CI$Severity, CFM2_4_order_row), ]
CFM5_17_CI_ordered <- CFM5_17_CI[match(CFM5_17_CI$Severity, CFM5_17_order_row), ]
...
names(CFM2_4_CI_final) <- c(" ", "% (95% CI)")
names(CFM5_17_CI_final) <- c(" ", "% (95% CI)")
...
## Part 2: Predicting difficulty Model (CFM2_4) from DD children

- This is going to be a generalized linear mixed effects model that is predicting reported difficulty (0,1) from age, domain, sex, domain * sex. 
- Child_ID will have their own random intercept
- HOH_ID will have their own random intercept
- Results will be preliminary since we need to make sure the data is cleaned! (HOH_ID)
- We will do model comparisons while increasing the complexity of the models
- We decided to only include DD since there is a large imbalence in favor of 0's if non-DD are included
- Thus, we can only generalized our findings to DD children!
...
# Check the difference in teh frequency of the outcome
table(CFM2_4_vars2_DD_long$Reported_Difficulty)
table(CFM2_4_vars_DD$Child_Gender)

# Number of children
length(unique(CFM2_4_vars2_DD_long$Child_ID))

# Run the model
CFM2_4_empty <- glmer(Reported_Difficulty ~ 1 + (1 | HOH_ID) + (1 | Child_ID), data = CFM2_4_vars2_DD_long, family = binomial) 

# Since the variance of HOH_ID is zero, it will be removed from the model
CFM2_4_empty2 <- glmer(Reported_Difficulty ~ 1 + (1 | Child_ID), data = CFM2_4_vars2_DD_long, family = binomial) 

# Obtain the ICC
icc(CFM2_4_empty2)

# Create a model with a covariate
CFM2_4_covariate <- glmer(Reported_Difficulty ~ Child_age + (1 | Child_ID), data = CFM2_4_vars2_DD_long, family = binomial) 

# Create a model with another covariate
CFM2_4_covariate2 <- glmer(Reported_Difficulty ~ Child_age + Child_Gender + (1 | Child_ID), data = CFM2_4_vars2_DD_long, family = binomial) 

# Create a model with the predictor of interest (Domain)
CFM2_4_main <- glmer(Reported_Difficulty ~ Child_age + Child_Gender + Domain + (1 | Child_ID), data = CFM2_4_vars2_DD_long, family = binomial) 

# Create a complex model with the interaction
CFM2_4_complex <- glmer(Reported_Difficulty ~ Child_age + Child_Gender * Domain + (1 | Child_ID), data = CFM2_4_vars2_DD_long, family = binomial) 
...
Anova(CFM2_4_main, type = "III")

# Due to the non-significan interaction, we will use effect coding on Domain
CFM2_4_vars2_DD_long$Domain <- factor(CFM2_4_vars2_DD_long$Domain)
contrasts(CFM2_4_vars2_DD_long$Domain) <- contr.sum(length(unique(CFM2_4_vars2_DD_long$Domain)))

# Fit the model with the effect-coded 'Domain' variable
CFM2_4_main2 <- glmer(Reported_Difficulty ~ Child_age + Child_Gender + Domain + (1 | Child_ID), 
                     data = CFM2_4_vars2_DD_long, family = binomial)

# Check the summary of the model
summary(CFM2_4_main2, cor = F)

# Spit the levels so we know what Domain1-8 mean (the intercept = CF10_Walking)
levels(CFM2_4_vars2_DD_long$Domain)

# Calcuating the means of reported difficulty
CFM2_4_Domain <- CFM2_4_vars2_DD_long %>%
  group_by(Domain) %>%
  summarise(Percentage = mean(Reported_Difficulty)*100) 

CFM2_4_Domain_order <- as.character(unlist(select(arrange(CFM2_4_Domain, desc(Percentage)), Domain)))

# Creating the graphs
CFM2_4_Domain %>%
  mutate(Domain = factor(Domain,
                         levels = CFM2_4_Domain_order)) %>%
  ...
  coord_flip() + 
  theme_classic() +
  labs(title = paste0("Percentage of at least some difficulty\nby functional domain CFM 2-4 (n=", nrow(CFM2_4_vars_DD),")"),
       x = "Functional Domain",
       y = "Percentage (%)") +
...
- We used model comparison to test the following models agains each other
    - Empty model
    - Predictor: Age
    - Predictors: Age and Sex
    - Predictors: Age, Sex, Domain
    - Predictors: Age, Sex, Domain, Sex * Domain
- The mode with main effects and no interactions was the best fit
- Using type III SS, there was a significant fixed effect of Domain (χ2(8)= 39.51, p <.001) and no fixed effects of age (χ2(1)= 1.90, p = .17) nor gender (χ2(1)= 0.03, p = .86).
- To investigate the log odd differences in the levels of Domain, effect coding was used. This is a way of formatting the 'means' by the levels, which are then compared to the average mean of all the levels- that functions as the reference.
- According to ChatGPT, the intercept represents the log-odds of CF10_Walking, which is significant, here is a list of all significant domains in descending order:
    - CF11_Fine_Motor
    - CF10_Walking
    - CF12_Understanding
    - CF14_Learning
...
... Either read about effect coding, review stats notes, or go back to pairwise comparison of estimated marginal means!
