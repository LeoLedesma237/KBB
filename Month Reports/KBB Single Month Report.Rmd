...
```{r Enter month and year, echo = FALSE}
# Enter a value for both (Numeric)
Month = 7
Year = 2024   

# Calculate values for next month
if(Month == 12) {
  
  Month = "12"
  End.Month = "01"
  End.Year = paste(Year + 1)
  Year = paste(Year)
  
} else if (Month < 10) {
  
  End.Month = paste("0",Month + 1,sep="")
  End.Year = Year
...
```{r Filter the data by the dates chosen, echo = FALSE}
start.date = paste(Year,"-",Month,"-01",sep="")
end.date = paste(End.Year,"-",End.Month,"-01",sep="")

# Filter by start and end date (encompasses a whole month)
Comprehensive.Screener.data <- Comprehensive.Screener.data %>%
  filter(between(Date_of_Evaluation,
                 start.date, 
                 end.date))
...
cat("This report is for the month of ", Month, ",",Year,".\n\n", sep="")
...
These errors occurred before code was created to identify the children and prevent them from being recruited. This code is now active thus only the 'Incorrect Screener' number should change from now on. 
...
- **Manual: Matched Half-Siblings**: Children that we can recruit into the study if we struggle to find full sibling pairs (none have been given an ID yet).
- **Manual: Matched Siblings**: Current DD and noDD sibling pairs that have been recruited.
...
data.frame(Frequency = cbind(table(Comprehensive.Screener.data$KBB_DD_status_Broken))) %>%
  kbl(caption = "Frequency of DD by type") %>%
  kable_paper(full_width = F)
