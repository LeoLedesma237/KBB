# This script is designed to plot MMN data
...
# SpecDatDir = 'C:/Users/lledesma.TIMES/Documents/KBB/Data/MODIFIED_DS/03_MMN_Inscapes/03_ERP_CSVs/'
# PlotDir = 'C:/Users/lledesma.TIMES/Documents/KBB/Data/MODIFIED_DS/03_MMN_Inscapes/04_Plotted_MMN/'
...
all_MMN_files <- list.files(path = SpecDatDir)
all_MMN_fullname <- list.files(path = SpecDatDir, full.names = TRUE)

# Create a data frame that is able to organized this information for each subject
full_dat <- data.frame(
  all_files = all_MMN_files,
  all_files_pathways = all_MMN_fullname
)

# Add more variables of interest to the dataframe
full_dat <- full_dat %>%
  mutate(ID = sub("_.*", "", all_files),
         Conditions = sub(".*MMN_ERPs_(.*)\\.txt", "\\1", all_files),
         Task = ifelse(grepl("Tone", Conditions), "Tone Task", "Phoneme Task"),
         Trial_Type = ifelse(grepl("Non_Target", Conditions), "Standard", "Deviant")) 
...
for(ii in 1:length(unique_IDs)) {
  ...
  long_dat <- one_person %>%
    ...
    mutate(Time = as.numeric(Time),
           Task = factor(Task, levels = c("Tone Task", "Phoneme Task")))
  ...
  # Save the plot
  ggsave(paste0(PlotDir,current_ID,"_MMN.png"), plot = last_plot(), width = 3.4, height = 3)
}
