...
full_data <- read_excel("full_data.xlsx") # Behavioral data raw scores (z-scores)
...
# Some minor data cleaning (include IDENTIFIABLE INFORMATION)
trackerID2 <- select(trackerID, full_child_ID = Child_ID, Child_ID = ID, Child_First_Name, Child_Last_Name, HOH_ID)
...
full_data2 <- drop_na(full_data2, Child_ID)
```
...
# Load in the data (MIRT data scored- much better!)
...
```{r include = F}
# Get some descriptives on full data
length(unique(full_data2$Child_ID))

# Get a sense of the dimensions
dim(full_data2)

# Get the proportion of missing data for each variable
round(colMeans(is.na(full_data2)),2)
```
...
cat('\n - First Name:', data$Child_First_Name, '\n')
cat('\n - Last Name:', data$Child_Last_Name, '\n')
cat('\n - Age:',data$Age,'\n')
cat('\n - Sex:',data$Sex,'\n')
cat('\n - HOH ID:', data$HOH_ID, '\n')
